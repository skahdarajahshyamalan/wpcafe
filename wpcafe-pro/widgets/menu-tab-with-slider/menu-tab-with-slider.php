...
class Menu_Tab_With_Slider extends Widget_Base{
	...
	public function get_name() {
		return 'wpc-menu-tab-with-slider';
	}
	...
	public function get_title() {
		return esc_html__('WPC Menu Tab With Slider Pro', 'wpcafe-pro');
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'section_tab',
			[
				'label' => esc_html__('WPC Menu Tab With Slider', 'wpcafe-pro'),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'         => Controls_Manager::COLOR,
				'condition' => ['food_tab_menu_style' => ['style-2', 'style-3']],
				'selectors'     => [
	...
	protected function render() {
		...
		?>
		<div class="wpc-nav-shortcode wpc-food-tab-wrapper wpc-widget-wrapper main_wrapper_<?php echo esc_attr($unique_id); ?>" data-id="<?php echo esc_attr($unique_id); ?>">
			<ul class="wpc-nav">
				<?php
					...
					foreach ($food_menu_tabs as $tab_key => $value) {
						...
						<li>
							<a href='#' class='wpc-tab-a <?php echo esc_attr($active_class); ?>' data-id='tab<?php echo esc_attr($this->get_id()) . "-" . $value['_id']; ?>'
								data-cat_id='<?php echo intval( $cat_id ) ?>'>
			...
			<div class="wpc-tab-content wpc-food-wrapper">
				<?php
				if( is_array( $food_menu_tabs ) ){
					foreach ($food_menu_tabs as $content_key => $value) {
						...
						data-cat_id='<?php echo esc_attr( $cat_id ) ; ?>'>
							<div class="tab_template_<?php echo esc_attr( $cat_id ) ; ?>"></div>
							<div class="template_data_<?php echo esc_attr( $cat_id ) ; ?>">
								<?php
                                    $products_args = array(
                                        'post_type'     => 'product',
